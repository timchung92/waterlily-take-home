import stateNonMetroRatesData from '../data/stateNonMetroCareEnvironmentCost.json';
...
export function getState(
  zipString: string,
  returnStateCode?: boolean,
): string | undefined {
  /* Ensure we have exactly 5 characters to parse */
  if (zipString.length !== 5 && zipString.length !== 4) {
    console.error('Must pass a 4 or 5-digit zipcode.');
    return undefined;
  }
...
type StateRateData = {
  [state: string]: CareEnvironmentData;
};

export function isValidState(state: string | undefined): state is State {
  if (!state) {
    return false;
  }
  return Object.values(State).includes(state as State);
}

export function getStateNonMetroRate(
  state: string | undefined,
): CareEnvironmentData | undefined {
  if (!isValidState(state)) {
    console.error('Must pass a valid state.');
    return undefined;
  }
  return (stateNonMetroRatesData as StateRateData)[state];
}
