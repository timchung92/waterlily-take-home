...
interface AddPolicyButtonProps {
  ...
  policyTypeSelectHandler: (policyType: FundingPolicyType) => void;
}

export function AddPolicyButton(props: AddPolicyButtonProps) {
  return (
    <>
            ...
            <div className="w-56 shrink rounded-xl bg-white p-2 text-base font-semibold leading-6 text-gray-900 shadow-lg ring-1 ring-gray-900/5">
              {fundingPolicyTypeDefList.map(policyType => (
                <button
                  ...
                  onClick={() =>
                    props.policyTypeSelectHandler(policyType.value)
                  }
                >
                  {policyType.label}
                </button>
